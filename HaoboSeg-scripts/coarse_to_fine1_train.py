...
x_train_dir = "train_vals/all_set_f_14Jun/"
y_train_dir = "train_vals/all_set_m_14Jun/"
...
def train_step(x, y_true, optim, optim2, coarse_model, model):
    optim.zero_grad()
    # model.train()
    y_pred_coarse, deep_sup1, deep_sup2, deep_sup3 =  coarse_model(x)
    loss1_o = criterion(y_pred_coarse, y_true)
    loss1_d1 = criterion(deep_sup1, y_true)
    loss1_d2 = criterion(deep_sup2, y_true)
    loss1_d3= criterion(deep_sup3, y_true)
    loss1 = (loss1_o + loss1_d1 + loss1_d2 +  loss1_d3)
    loss1.backward()
    optim.step()
    optim.zero_grad()
    y_pred, deep_sup1f, deep_sup2f, deep_sup3f = model(torch.cat([y_pred_coarse.detach(), x], dim=1))
    loss2_o = criterion(y_pred, y_true)
    loss2_d1 = criterion(deep_sup1f, y_true)
    loss2_d2 = criterion(deep_sup2f, y_true)
    loss2_d3= criterion(deep_sup3f, y_true)
...
# print(model)
    ...
    coarse_model = coarse_Unet(in_ch=3)
    coarse_model.cuda(gpu)
    model = coarse_Unet(in_ch=6)
    model.cuda(gpu)
    ...
    for epoch in range(EPOCHS):
        ...
        if gpu==0:
            print(f"EPOCH: {epoch} Train_loss:  {train_loss:.4f}, Train_dice_whole: {train_dice_2:.4f}, lr: {optim.param_groups[0]['lr']}")
        ...
        for idx, (x_batch_val, y_batch_val, _) in enumerate(val_dataloader):
            # train_loss, train_dice_0, train_dice_1 = train_step(x_batch_train.cuda(), y_batch_train.cuda())
            val_loss, val_dice2 = val_step(x_batch_val.to(device, non_blocking=True), y_batch_val.to(device, non_blocking=True), coarse_model, model)
        ...
        # Writes scores in this epoch to tensorboard
            if writer is not None:
                ...
                writer.add_scalars('Whole Dice', {'train': train_dice_2,
